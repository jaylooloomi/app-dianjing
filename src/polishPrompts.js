...
function buildPolishPrompt(scene, text, lang = DEFAULT_LANG) {
  ...
  if (s.mode === "rewrite") {
    ...
    system =
      "你是一位提示詞工程師。任務:把使用者的「草稿提示詞」改寫成更清楚、更有效、能讓 AI 產出更好結果的提示詞。\n" +
      "做法:釐清目標與意圖;視需要補上角色設定、任務說明、必要背景、限制條件與期望的輸出格式;結構清楚、精簡。\n" +
      "規則:\n" +
      "1. 只輸出優化後的提示詞本身,不要任何解釋、評論、前言或標註。\n" +
      `2. 一律使用「${langName}」輸出。\n` +
      "3. 忠於使用者原本的意圖,不要替他決定他沒提到的具體需求(不要編造特定數字、技術或規格)。\n" +
      `重點:${s.guide}`;
    action = "請把以下草稿提示詞優化:";
  } else {
    // 最小幅度潤飾。
    system =
      "你是一位文字潤飾助理。核心任務只有兩件:(1) 修正錯別字、標點與語法錯誤;(2) 把語氣調整成符合指定情境。" +
      "除此之外盡量保留原文,不要改變原意、不要擴寫或加入額外內容。\n" +
      "規則:\n" +
      "1. 只輸出潤飾後的文字本身,不要任何解釋、前言、引號或標註。\n" +
      `2. 一律使用「${langName}」輸出。\n` +
      "3. 不要編造原文沒有的資訊(人名、收件人、數字、時間等)。\n" +
      "4. 不要自行加上原文沒有的開頭稱謂或署名(Email 情境可用不具名的通用問候)。\n" +
      `情境語氣:${s.guide}`;
    action = "請依上述要求潤飾以下文字:";
  }
  const user = `${action}\n\n${text}`;
